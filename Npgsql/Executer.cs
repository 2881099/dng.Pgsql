...
namespace Npgsql {
	public partial class Executer : IDisposable {
		...
		void LoggerException(ConnectionPool pool, NpgsqlCommand cmd, Exception e, DateTime dt, string logtxt, bool isThrowException = true) {
		...
		private (Connection2 conn, NpgsqlTransaction tran) PrepareCommand(ConnectionPool pool, NpgsqlCommand cmd, CommandType cmdType, string cmdText, NpgsqlParameter[] cmdParms, ref string logtxt) {
			DateTime dt = DateTime.Now;
