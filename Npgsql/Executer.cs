...
namespace Npgsql {
	public partial class Executer : IDisposable {
		...
		public NpgsqlConnectionPool MasterPool { get; }
		public List<NpgsqlConnectionPool> SlavePools { get; } = new List<NpgsqlConnectionPool>();
		private int slaveUnavailables = 0;
		private object slaveLock = new object();
		private Random slaveRandom = new Random();

		void LoggerException(NpgsqlConnectionPool pool, NpgsqlCommand cmd, Exception e, DateTime dt, string logtxt, bool isThrowException = true) {
		...
		/// <param name="cmdParms"></param>
		/// <returns></returns>
		public object[][] ExecuteArray(CommandType cmdType, string cmdText, params NpgsqlParameter[] cmdParms) {
		...
		private (NpgsqlTransaction tran, NpgsqlCommand cmd) PrepareCommand(CommandType cmdType, string cmdText, NpgsqlParameter[] cmdParms, ref string logtxt) {
			DateTime dt = DateTime.Now;
